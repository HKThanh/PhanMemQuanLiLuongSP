...
//	}
	...
	public JPanel getPCCDUI() {
		...
		String[] header = { "Mã sản phẩm", "Tên sản phẩm", "Số lượng", "Số lượng công đoạn hiện có",
		...
		pnlCacCongDoan.setBorder(new TitledBorder(null, "C\u00E1c c\u00F4ng \u0111o\u1EA1n hi\u1EC7n c\u00F3",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));
		pnlCacCongDoan.setBounds(0, 240, 1264, 340);
		pnlPCCD.add(pnlCacCongDoan);
		pnlCacCongDoan.setLayout(null);

		String[] header_CongDoan = { "Tên sản phẩm", "Mã công đoạn", "Tên công đoạn", "Số lượng sản phẩm",
				"Số lượng công nhân dự kiến", "Giá tiền (VND)", "Trạng thái", "Ngày bắt đầu", "Ngày kết thúc",
				"Công đoạn tiên quyết" };
	 ...
	 */
	private void layDSSanPhamTuDB() throws RemoteException {
//		sp_DAO = new SanPham_Impl();
	 ...
	 * @throws RemoteException
	 */
	private void layDSCongDoanTuDBTheoMaSP(String maSP) throws RemoteException {
		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("dd/MM/yyyy");
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (object.equals(tableCongDoan)) {
			...
			List<BangPhanCongCN> listBPCCN = null;
			try {
				listBPCCN = bPCCN_DAO.getDSPhanCongCongDoanTheoMaCD(maCD);
			} catch (RemoteException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			...
			if (listBPCCN.size() != 0) {
				JOptionPane.showMessageDialog(null,
						"Lưu ý công đoạn này hiện đã phân công, Không thể thực hiện chỉnh sửa");
			} else {
	...
	/**
	 * Phương thức cập nhật combobox công đoạn tiên quyết khi có một công đoạn được
	 * thêm vào sản phẩm
	 * 
	 * @param maSP
	 * @throws RemoteException
	 */
	private void capNhatCBOTienQuyet(String maSP) throws RemoteException {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnThem)) {
			...
			try {
				if (validation()) {
					...
					LocalDate ngayBatDau = dcNgayBatDau.getDate().toInstant().atZone(ZoneId.systemDefault())
							.toLocalDate();
					LocalDate ngayKetThuc = dcNgayKetThuc.getDate().toInstant().atZone(ZoneId.systemDefault())
							.toLocalDate();
					String maCD = taoMaCongDoan(maSP);
					String tienQuyet = cboMaCDTienQuyet.getSelectedItem().toString();
					SanPham sp = null;
					try {
						sp = sp_DAO.getMotSanPham(maSP);
					} catch (RemoteException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					...
					CongDoan cd = null;
					if (tienQuyet.equals(" ")) {
						cd = new CongDoan(maCD, tenCD, soLuongSP, soLuongCN, giaTien, ngayBatDau, ngayKetThuc, false,
								"", sp);
					} else {
						cd = new CongDoan(maCD, tenCD, soLuongSP, soLuongCN, giaTien, ngayBatDau, ngayKetThuc, false,
								tienQuyet, sp);
		...
		if (o.equals(btnXoa)) {
			String maCD = modelCongDoan.getValueAt(tableCongDoan.getSelectedRow(), 1).toString();
			int luaChon = JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xoá công đoạn này", "Lưu ý",
		...
		if (o.equals(btnHoanTat)) {
			...
			try {
				...
				int luaChon = JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn sửa sản phẩm này không?", "Lưu Ý",
						JOptionPane.YES_NO_OPTION);
	...
	/**
	 * Phương thức cập nhật số công đoạn hiện có cho sản phẩm mỗi khi thêm một công
	 * đoạn
	 * 
	 * @param maSP
	 * @throws RemoteException
	 */
	private void capNhatSoCongDoanChoSP(String maSP) throws RemoteException {
		SanPham sp = sp_DAO.getMotSanPham(maSP);
	 ...
	 * @throws RemoteException
	 */
	private String taoMaCongDoan(String maSP) throws RemoteException {
		return maSP + (cd_DAO.getDSCongDoanTheoMaSP(maSP).size() + 1);
	}

	private boolean validation() throws RemoteException {
		...
		try {
			...
			if (soLuongSP <= 0 && soLuongSP > sp.getSoLuong()) {
				thongBaoLoi(txtSoLuongSP,
						"Số lượng sản phẩm của công đoạn phải là Số > 0 và không được quá số lượng sản phẩm ban đầu");
				return false;
			}
		...
		Date ngayThanhLiHD = Date
				.from(hd.getNgayThanhLyHopDong().atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
	...
	/**
	 * Phương thức thông báo lỗi, khi không hợp lệ sẽ trỏ đến jtextfield không hợp
	 * lệ
	 * 
	 * @param txt
	 * @param mess
