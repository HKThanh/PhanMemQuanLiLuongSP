...
public class ChiTietCN_GUI extends JFrame {
	...
	public JPanel getChiTietCN_GUI() {
		...
		String headerChiTiet[] = { "Mã Chấm Công", "Ngày Chấm", "Số Giờ Tăng Ca", "Ca Làm", "Có Mặt", "Có Phép",
				"Sản Lượng", "Công Đoạn", "Sản Phẩm", "Đơn giá", "Tiền Công", "Ghi chú" };
...
//		txtPhuCap_1.setBounds(705, 631, 200, 25);
	...
	public void khoiTaoChiTietLuong(String maCN, int thang, int nam, String Xuong, double khauTru) throws RemoteException {
		...
		for (BangChamCongCN bcc : dsBCC) {
			...
			modelTableChiTietBangLuong.addRow(new Object[] { bcc.getMaChamCongCN(), ngayChamFormatted,
					bcc.getSoGioTangCa(), bcc.getCaLam() == 1 ? "Sáng" : "Tối", bcc.isVangMat() ? false : true,
					bcc.isCoPhep() && bcc.isVangMat() ? true : false, bcc.getSanLuong(),
					...
					decimalFormat.format(bcc_DAO.layGiaTienSanPhamTheoBCC(bcc)), decimalFormat.format(tienCong), "" });
		}
