...
public class ChiTietNV_GUI extends JFrame {
	...
	private void khoiTaoChiTietLuong(String maNV,int Thang,int Nam,String mabp,double khauTru) {
        ...
        txtThuong.setText((decimalFormat.format(bcc_DAO.getTongSoGioTangCaCua1NV(nv.getMaNV(),Thang,Nam)*tienCong1Gio*2))+" VNĐ");
        for(BangChamCongNV bcc : listBCC) {
            ...
            modelTableChiTietBangLuong.addRow(new Object[] {bcc.getMaChamCongNV(),dtf.format(bcc.getNgayCham()),bcc.getSoGioTangCa(),bcc.getCaLam()==1?"Sáng":"Tối",bcc.isCoMat(),bcc.isVangMat(),bcc.isCoPhep(),decimalFormat.format(tienCong),bcc.getGhiChu()});
        }
