...
public class LuongCongNhan_GUI extends JFrame implements ActionListener, MouseListener {
	...
	public JPanel getLuongCNGUI() throws RemoteException {
		...
		String headerDSLuongCN[] = { "Mã Công Nhân", "Họ Tên", "Lương cơ bản", "Lương sản phẩm", "Số ngày làm",
				"Phụ Cấp", "Khấu trừ", "Bảo hiểm xã hội", "Tổng lương", "Đã Tính Lương", "Ghi chú" };
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (rowdk < 0) {
					...
					if (kiemtraCoFalseKhong()) {
						...
						int chon = JOptionPane.showConfirmDialog(null, "Bạn có muốn chọn tất cả công nhân không?",
		...
		btnTinhLuongCN.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (rows.length <= 0 || rowdk < 0) {
					...
					if (rows.length > 0) {
						if (tblDSLuongCN.getRowSorter() != null) {
							TableRowSorter<TableModel> sorter = (TableRowSorter<TableModel>) tblDSLuongCN
									.getRowSorter();
							for (int i = 0; i < rows.length; i++) {
								int row = rows[i];
								if (row < tblDSLuongCN.getRowCount()) {
									...
									try {
										khauTrudk = Double.parseDouble(modelTableDSLuongCN.getValueAt(modelRow, 6)
												.toString().replace(",", ""));
									...
									boolean coLuongRoi = Double.parseDouble(
											modelTableDSLuongCN.getValueAt(modelRow, 8).toString().replace(",", "")) > 0
													? true
													: false;
									if (daTinhLuong == true && coLuongRoi == true) {
										int luaChon = JOptionPane.showConfirmDialog(null, "Công Nhân "
												+ modelTableDSLuongCN.getValueAt(row, 1).toString() + " có mã số "
												+ modelTableDSLuongCN.getValueAt(row, 0).toString()
												+ " đã có lương. Bạn có muốn tính lại lương cho công nhân này không?",
												"Thông báo", JOptionPane.YES_NO_OPTION);
									...
									if (daTinhLuong == false) {
										...
										double luongCoBan = Double.parseDouble(modelTableDSLuongCN
												.getValueAt(modelRow, 2).toString().replace(",", ""));
										double luongSanPham = Double.parseDouble(modelTableDSLuongCN
												.getValueAt(modelRow, 3).toString().replace(",", ""));
										double soNgayLam = Double.parseDouble(modelTableDSLuongCN
												.getValueAt(modelRow, 4).toString().replace(",", ""));
										double phuCap = Double.parseDouble(modelTableDSLuongCN.getValueAt(modelRow, 5)
												.toString().replace(",", ""));
										double khauTru = Double.parseDouble(modelTableDSLuongCN.getValueAt(modelRow, 6)
												.toString().replace(",", ""));
...
//												bc_DAO = new BangChamCongCN_Impl();
							...
							for (int i = 0; i < rows.length; i++) {
								...
								boolean daTinhLuong = (boolean) modelTableDSLuongCN.getValueAt(row, 9);
								boolean coLuongRoi = Double.parseDouble(
										modelTableDSLuongCN.getValueAt(row, 8).toString().replace(",", "")) > 0 ? true
												: false;
								if (daTinhLuong == true && coLuongRoi == true) {
									int luaChon = JOptionPane.showConfirmDialog(null, "Công Nhân "
											+ modelTableDSLuongCN.getValueAt(row, 1).toString() + " có mã số "
								...
								if (daTinhLuong == false) {
									...
									double luongCoBan = Double.parseDouble(
											modelTableDSLuongCN.getValueAt(row, 2).toString().replace(",", ""));
									double luongSanPham = Double.parseDouble(
											modelTableDSLuongCN.getValueAt(row, 3).toString().replace(",", ""));
									double soNgayLam = Double.parseDouble(
											modelTableDSLuongCN.getValueAt(row, 4).toString().replace(",", ""));
									double phuCap = Double.parseDouble(
											modelTableDSLuongCN.getValueAt(row, 5).toString().replace(",", ""));
									double khauTru = Double.parseDouble(
											modelTableDSLuongCN.getValueAt(row, 6).toString().replace(",", ""));
									int rowas = tblThangLuongCN.getSelectedRow();
									int thang = Integer
											.parseInt(modelTableThangLuongCN.getValueAt(rowas, 0).toString());
...
//			bl_DAO = new BangLuongCN_Impl();
			...
			if (bLuongCN == null) {
				double lsp = 0;
				List<BangChamCongCN> list = bc_DAO.getBangCCCNTheomaCNThangNam(cn.getMaCN(), thang, nam);
				for (BangChamCongCN bc : list) {
					lsp += bc_DAO.layGiaTienSanPhamTheoBCC(bc) * bc.getSanLuong();
				}

				Object[] row = { cn.getMaCN(), cn.getHo() + " " + cn.getTen(), decimalFormat.format(cn.getLuongCoBan()),
						decimalFormat.format(lsp), bc_DAO.laySoNgayDiLamCuaCNTheoThangNam(cn.getMaCN(), thang, nam),
						decimalFormat.format(cn.getPhuCap()), 0, 0, 0, false, "" };
				...
				Object[] row = { bLuongCN.getCn().getMaCN(), bLuongCN.getCn().getHo() + " " + bLuongCN.getCn().getTen(),
						decimalFormat.format(cn.getLuongCoBan()), decimalFormat.format(lsp),
						bc_DAO.laySoNgayDiLamCuaCNTheoThangNam(cn.getMaCN(), thang, nam),
						decimalFormat.format(cn.getPhuCap()), decimalFormat.format(bLuongCN.getTienPhat()),
						decimalFormat.format(bLuongCN.getBhxh()), decimalFormat.format(bLuongCN.getLuongTong()), true,
						"Đã có bảng lương" };
