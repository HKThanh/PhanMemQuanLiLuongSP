...
//		createMenuGUI();
	...
	protected JPanel createGUI() throws RemoteException {
		...
		BoPhan_DAO boPhanDao = Initiate.boPhan_DAO;
		chamCongNVDao = Initiate.bangChamCongNV_DAO;
			...
			public void itemStateChanged(ItemEvent e) {
				...
				try {
					...
					if (chkCaSang.isSelected() == true) {
						...
						dsCongLamViec = chamCongNVDao.getBangCCTheoNgayBPCa(ngayChamCong,
								dsBP.get(boPhanIndex).getMaBoPhan(), 1);
						...
						dsCongLamViec = chamCongNVDao.getBangCCTheoNgayBPCa(ngayChamCong,
								dsBP.get(boPhanIndex).getMaBoPhan(), 2);
						dsTen = layDsTen(dsNVBPCa);
						AutoCompleteDecorator.decorate(txtTimKiemTen, dsTen, false);
						if (kiemTraChamCong(dsCongLamViec)) {
							clearTable();
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (e.getStateChange() == ItemEvent.SELECTED) {
					...
					try {
						...
						if (source == chkCaSang) {
							chkCaToi.setSelected(false);
							dsNVBPCa = nhanVienDao.getListNVtheoBPCa(1,
									dsBP.get(cboBoPhan.getSelectedIndex()).getMaBoPhan());
							dsCongLamViec = chamCongNVDao.getBangCCTheoNgayBPCa(ngayChamCong,
									dsBP.get(cboBoPhan.getSelectedIndex()).getMaBoPhan(), 1);
							dsTen = layDsTen(dsNVBPCa);
							AutoCompleteDecorator.decorate(txtTimKiemTen, dsTen, false);
							if (kiemTraChamCong(dsCongLamViec)) {
								clearTable();
							...
							dsNVBPCa = nhanVienDao.getListNVtheoBPCa(2,
									dsBP.get(cboBoPhan.getSelectedIndex()).getMaBoPhan());
							dsCongLamViec = chamCongNVDao.getBangCCTheoNgayBPCa(ngayChamCong,
									dsBP.get(cboBoPhan.getSelectedIndex()).getMaBoPhan(), 2);
							dsTen = layDsTen(dsNVBPCa);
							AutoCompleteDecorator.decorate(txtTimKiemTen, dsTen, false);
							if (kiemTraChamCong(dsCongLamViec)) {
								clearTable();
		...
		String[] cols_nhanVien = { "Mã nhân viên", "Họ đệm", "Tên nhân viên", "Ca làm", "Đi làm", "Vắng phép",
				"Vắng không phép", "Số giờ tăng ca", "Ghi chú" };
			...
			public void tableChanged(TableModelEvent e) {
				if (tblDsNV.getRowCount() == dsCongLamViec.size()) {
					if (e.getType() == TableModelEvent.UPDATE
							&& (e.getColumn() == 4 || e.getColumn() == 5 || e.getColumn() == 6) || e.getColumn() == 7
							|| e.getColumn() == 8) {
						...
						if (!(modelDsNV.getValueAt(row, 7).toString().equals("")
								|| modelDsNV.getValueAt(row, 7).toString().equals("0"))) {
							dsCongLamViec.get(row)
									.setSoGioTangCa(Integer.parseInt(modelDsNV.getValueAt(row, 7).toString()));
						}
		...
		txtTimKiemTen.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_BACK_SPACE) {
					if ((txtTimKiemTen.getSelectionStart() == 1
							&& txtTimKiemTen.getSelectionEnd() == txtTimKiemTen.getText().length())
							|| (txtTimKiemTen.getSelectionStart() == 0
									&& txtTimKiemTen.getSelectionEnd() == txtTimKiemTen.getText().length())) {
	...
	public ArrayList<BangChamCongNV> taoDefaultDsChamCong(ArrayList<NhanVien> dsNVBPCa) {
		...
		for (NhanVien nv : dsNVBPCa) {
			...
			try {
				...
				String ghiChu = "";
				BangChamCongNV bangChamCongNV = new BangChamCongNV(maCC, nv, ngayChamCong, soGioTangCa, caLam, coMat,
						coPhep, vangMat, ghiChu);
		...
		for (int i = 0; i < dsNVBPCa.size(); i++) {
			for (BangChamCongNV bangCC : dsCongLamViec) {
				if (dsNVBPCa.get(i).getMaNV().equals(bangCC.getNv().getMaNV())) {
					Object[] rowData = { dsNVBPCa.get(i).getMaNV(), dsNVBPCa.get(i).getHo(), dsNVBPCa.get(i).getTen(),
							layCaLamViec(dsNVBPCa.get(i)), new JRadioButton(), new JRadioButton(), new JRadioButton(),
							bangCC.getSoGioTangCa(), bangCC.getGhiChu() };
	...
	public String phatSinhMaCC(LocalDate ngayChamCong, String maNV) {
		...
		String ma = dayFormat.format(Date.valueOf(ngayChamCong)) + monthFormat.format(Date.valueOf(ngayChamCong))
				+ yearFormat.format(Date.valueOf(ngayChamCong)) + maNV;
		return ma;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		int i = 0;
		if (o == btnHoanTat) {
			if (JOptionPane.showConfirmDialog(frame, "Bạn hãy xác nhận hoàn thành bảng chấm công", "Xác Nhận",
		...
		if (o == btnCapNhat) {
			if (JOptionPane.showConfirmDialog(frame, "Hãy xác nhận cập nhật bảng chấm công", "Xác Nhận",
					JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				for (BangChamCongNV bangChamCong : dsCongLamViec) {
					try {
				...
				if (dsNVTimKiem.size() == 0) {
					...
					for (int n = 0; n < dsNVTimKiem.size(); n++) {
						for (BangChamCongNV bangCC : dsBangCCTimKiem) {
							if (dsNVTimKiem.get(n).getMaNV().equals(bangCC.getNv().getMaNV())) {
								Object[] rowData = { dsNVTimKiem.get(n).getMaNV(), dsNVTimKiem.get(n).getHo(),
										dsNVTimKiem.get(n).getTen(), layCaLamViec(dsNVTimKiem.get(n)),
					...
					tblDsNV.getModel().addTableModelListener(new TableModelListener() {
						...
						public void tableChanged(TableModelEvent e) {
							if (tblDsNV.getRowCount() < dsCongLamViec.size()) {
								if (e.getType() == TableModelEvent.UPDATE
										&& (e.getColumn() == 4 || e.getColumn() == 5 || e.getColumn() == 6)
										|| e.getColumn() == 7 || e.getColumn() == 8) {
									int row = e.getFirstRow();
									if (((JRadioButton) modelDsNV.getValueAt(row, 4)).isSelected() == true) {
									...
									if (!(modelDsNV.getValueAt(row, 7).toString().equals("")
											|| modelDsNV.getValueAt(row, 7).toString().equals("0"))) {
										dsBangCCTimKiem.get(row).setSoGioTangCa(
												Integer.parseInt(modelDsNV.getValueAt(row, 7).toString()));
									}
									...
									for (BangChamCongNV bangChamCong : dsCongLamViec) {
										if (bangChamCong.getMaChamCongNV()
												.equals(dsBangCCTimKiem.get(row).getMaChamCongNV())) {
